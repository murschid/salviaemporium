(function ($) {
    ...
    $('.cd-trigger').on('click', function (event) {
        ...
        $.ajax({
            url: baseurl + 'action/getprobyajax',
            method: 'POST',
            type: 'html',
    ...
    $('.qty-btn').on('click', function (e) {
        ...
        var value = newVal;
        var rowid = $button.parent().find('input').attr('id');
        var price = $button.parent().find('input').attr('for');
        $.ajax({
            url: baseurl + 'cart/cart_update',
            method: 'POST',
            type: 'text',
            data: {'quantity': value, 'row_id': rowid, 'price': price},
            success: function (result) {
                $('.' + rowid).html('৳' + result * price);
    ...
    $('.pro-qty-two').append('<a class="inc qty-btn-two"><i class="pe-7s-plus"></i></a>');
    $('.pro-qty-two').prepend('<a class= "dec qty-btn-two"><i class="pe-7s-less"></i></a>');
    $('.qty-btn-two').on('click', function (e) {
        e.preventDefault();
        var $button = $(this);
        var oldValue = $button.parent().find('input').val();
        if ($button.hasClass('inc')) {
    ...
    $('#price-amount').val('Price: ' + '৳' + $('#price-range').slider('values', 0) + ' - ৳' + $('#price-range').slider('values', 1));
